...
void PlayerViewModel::shoot(const QPointF& direction)
{
    if (canShoot() && !direction.isNull()) {
        ...
        if (m_stats.wheelMode && m_stats.shotgunMode) {
            ...
            // 正常射击模式：只向指定方向发射一颗子弹
            m_bulletViewModel->createBullet(m_stats.position, direction, 200, m_stats.bulletDamage);
        }
...
void PlayerViewModel::shootInShotgunPattern(const QPointF& direction)
{
    ...
    for (const auto& dir : directions) {
        m_bulletViewModel->createBullet(m_stats.position, dir, 200, m_stats.bulletDamage);
    }
...
void PlayerViewModel::shootInWheelShotgunCombination()
{
    ...
    for (const auto& mainDirection : directions) {
        ...
        // 向三个方向发射子弹
        m_bulletViewModel->createBullet(m_stats.position, leftDir, 200, m_stats.bulletDamage);
        m_bulletViewModel->createBullet(m_stats.position, centerDir, 200, m_stats.bulletDamage);
        m_bulletViewModel->createBullet(m_stats.position, rightDir, 200, m_stats.bulletDamage);
    }
